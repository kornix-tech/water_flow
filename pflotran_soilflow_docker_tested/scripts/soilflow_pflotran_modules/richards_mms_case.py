...
def richards_mms_spatial_source_manifest(case: RichardsMmsCase) -> dict[str, object]:
    return {
        "schema_version": 1,
        "artifact_status": "MATRIX_ARTIFACT_READY_DECK_ACTIVE",
        ...
        "deck_adapter_blocker": (
            "Spatial adapter deck применяет cell-wise source и nonuniform initial profile; "
            "следующий риск - расширенная solver validation."
        ),
    }


def _richards_mms_cell_region_name(cell_id: int) -> str:
    return f"mms_cell_{cell_id:03d}"


def generate_richards_mms_spatial_adapter_input(case: RichardsMmsCase = RichardsMmsCase()) -> str:
    ...
    source_by_cell: dict[int, list[dict[str, float]]] = {cell_id: [] for cell_id in range(1, case.nz + 1)}
    for row in source_rows:
        source_by_cell[int(row["cell_id"])].append(row)

    lines = [
        "# Generated by soilflow_pflotran.py --mode _test --test richards_mms",
        "# Richards MMS spatial adapter candidate.",
        "# Nonuniform initial pressure and cell-wise source terms are expanded",
        "# into per-cell PFLOTRAN regions until a compact native table adapter is validated.",
        "",
        "SIMULATION",
    ...
    for cell_id, initial_row in enumerate(initial_rows, start=1):
        z_low = (cell_id - 1) * dz
        z_high = cell_id * dz
        region_name = _richards_mms_cell_region_name(cell_id)
        lines.extend(
            [
                f"REGION {region_name}",
                "  COORDINATES",
                "    0.d0 0.d0 " + pf_float(z_low),
                f"    {pf_float(case.length_x_m)} {pf_float(case.length_y_m)} {pf_float(z_high)}",
                "  /",
                "END",
                "",
                f"FLOW_CONDITION initial_{region_name}",
                "  TYPE",
                "    LIQUID_PRESSURE DIRICHLET",
                "  /",
                f"  LIQUID_PRESSURE {pf_float(initial_row['pressure_pa'])}",
                "END",
                "",
                f"FLOW_CONDITION source_{region_name}",
                "  TYPE",
        ...
        for source_row in source_by_cell[cell_id]:
            lines.append(f"        {pf_float(source_row['time_days'])} {pf_float(source_row['source_rate_cell_m3_day'])}")
        lines.extend(["  /", "END", ""])
    ...
    for cell_id in range(1, case.nz + 1):
        ...
        lines.extend(
            [
                ...
                "END",
                "",
                f"SOURCE_SINK source_{region_name}",
                f"  FLOW_CONDITION source_{region_name}",
                f"  REGION {region_name}",
                "/",
                "",
...
def write_richards_mms_spatial_adapter_artifacts(case: RichardsMmsCase, workdir: Path) -> None:
    deck_file = "pflotran_spatial_mms_candidate.in"
    manifest = {
        "schema_version": 1,
        "adapter_status": "SPATIAL_DECK_DEFAULT_ACTIVE",
        "deck_file": deck_file,
        "cell_count": case.nz,
        "source_mode": "per_cell_scaled_volumetric_rate",
        "initial_condition_mode": "per_cell_liquid_pressure",
        "default_runner_uses": "pflotran.in",
        "gate_status": "STRICT_CANDIDATE_CAN_GATE",
        "gate_note": "Spatial deck используется основным runner-ом; suite gate зависит от strict pressure/theta evaluator.",
    }
    (workdir / deck_file).write_text(generate_richards_mms_spatial_adapter_input(case), encoding="utf-8")
    (workdir / "richards_mms_spatial_adapter_manifest.json").write_text(
...
def richards_mms_adapter_summary(case: RichardsMmsCase) -> dict[str, object]:
    ...
    return {
        ...
        "richards_mms_adapter_deck_status": "PFLOTRAN_SPATIAL_ADAPTER_ACTIVE",
        "richards_mms_spatial_adapter_deck_status": "SPATIAL_DECK_DEFAULT_ACTIVE",
        "richards_mms_spatial_adapter_deck_file": "pflotran_spatial_mms_candidate.in",
...
def validate_richards_mms_adapter_artifacts(workdir: Path) -> dict[str, object]:
    ...
    spatial_adapter_manifest_path = workdir / "richards_mms_spatial_adapter_manifest.json"
    spatial_adapter_deck_path = workdir / "pflotran_spatial_mms_candidate.in"
    ...
    spatial_adapter_check = "SKIP"
    spatial_adapter_note = "Spatial adapter candidate deck еще не записан."
    spatial_adapter_status = ""
    spatial_adapter_deck_file = ""
    if spatial_adapter_manifest_path.exists() and spatial_adapter_deck_path.exists():
        try:
            adapter_manifest = json.loads(spatial_adapter_manifest_path.read_text(encoding="utf-8"))
            deck_text = spatial_adapter_deck_path.read_text(encoding="utf-8")
            expected_regions = [f"REGION {_richards_mms_cell_region_name(cell_id)}" for cell_id in range(1, cell_count + 1)]
            expected_sources = [
                f"SOURCE_SINK source_{_richards_mms_cell_region_name(cell_id)}"
                for cell_id in range(1, cell_count + 1)
            ]
            deck_shape_ok = all(item in deck_text for item in (*expected_regions, *expected_sources))
            manifest_shape_ok = int(adapter_manifest["cell_count"]) == cell_count
            spatial_adapter_check = "PASS" if deck_shape_ok and manifest_shape_ok else "FAIL"
            spatial_adapter_note = (
                "Spatial MMS adapter deck содержит per-cell regions/source sinks."
                if spatial_adapter_check == "PASS"
                else "Spatial MMS adapter deck не совпал с matrix manifest по cell coverage."
            )
            spatial_adapter_status = str(adapter_manifest.get("adapter_status", ""))
            spatial_adapter_deck_file = str(adapter_manifest.get("deck_file", ""))
        except (KeyError, TypeError, ValueError, json.JSONDecodeError) as exc:
            spatial_adapter_check = "FAIL"
            spatial_adapter_note = f"Spatial MMS adapter manifest не прошел schema check: {exc}"
    return {
        ...
        "richards_mms_spatial_adapter_check": spatial_adapter_check,
        "richards_mms_spatial_adapter_note": spatial_adapter_note,
        "richards_mms_spatial_adapter_deck_status": spatial_adapter_status,
        "richards_mms_spatial_adapter_deck_file": spatial_adapter_deck_file,
    }
...
def write_richards_mms_case_artifacts(case: RichardsMmsCase, workdir: Path) -> None:
    ...
    write_richards_mms_spatial_adapter_artifacts(case, workdir)
