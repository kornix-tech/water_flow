...
def run_test_suite(request: Request) -> JobCreated:
    command, run_name, output_dir = soilflow_cli.test_command(_application_settings(request), "all")
    job = _job_manager(request).submit("test-suite", command, output_dir, run_name)
...
def run_test(test_name: str, request: Request) -> JobCreated:
    command, run_name, output_dir = soilflow_cli.test_command(_application_settings(request), test_name)
    job = _job_manager(request).submit("test", command, output_dir, run_name)
    return JobCreated(job_id=job.id, status=job.status, run_name=job.run_name)
...
def run_visualization(run_name: str, request: Request) -> JobCreated:
    command, normalized_run_name, output_dir = soilflow_cli.visualization_command(_application_settings(request), run_name)
...
def submit_calculation_run(request: Request, calculation_id: int, run_name: str | None = None) -> JobCreated:
    ...
    job = _job_manager(request).submit("calculation", command, output_dir, command_run_name, calculation_id=calculation.id)
...
def list_jobs(request: Request) -> list[JobRead]:
    return [JobRead(**job.__dict__) for job in _job_manager(request).list_jobs()]


@router.get("/{job_id}", response_model=JobRead)
def get_job(job_id: str, request: Request) -> JobRead:
    return JobRead(**_job_manager(request).get_job(job_id).__dict__)
...
def cancel_job(job_id: str, request: Request) -> JobRead:
    return JobRead(**_job_manager(request).cancel(job_id).__dict__)
