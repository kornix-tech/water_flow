...
def _run_info(run_dir: Path, file_manager, *, include_files: bool = True) -> RunInfo:
    ...
    return RunInfo(
        ...
        path=str(run_dir),
        has_test_status=has_status_artifact(run_dir, "TEST_STATUS.txt"),
        has_suite_status=any(has_status_artifact(run_dir, filename) for filename in SUITE_STATUS_ARTIFACTS),
