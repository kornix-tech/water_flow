...
Если web-задание превышает `SOILFLOW_JOB_TIMEOUT_SECONDS`, backend завершает
дочерний процесс, пишет `[TIMEOUT]` в job log и сохраняет exit code `124`.
Для CLI/verification запусков PFLOTRAN отдельно доступен
`--solver-timeout-seconds` или `SOILFLOW_SOLVER_TIMEOUT_SECONDS`; при превышении
лимита verification status получает `TEST_STATUS=PFLOTRAN_TIMEOUT`.
Verification artifacts дополнительно публикуют `failure_stage`: `generation`,
`solver`, `parser` или `evaluator`, чтобы API/UI и suite CSV различали этап
сбоя без разбора текста лога. `parser` означает, что внешний solver уже
вернул artifacts, но TECPLOT/profile output не удалось прочитать как расчетный
контракт; `evaluator` остается для ошибок физической оценки после успешного
разбора output.
...
- `GET /api/results/runs/{run_name}/status` - текстовый статус расчета/теста.
  Endpoint отдает только обычный файл внутри run-директории, не следует по
  symlink и применяет inline-лимит 2 MiB.
...
- `GET /api/results/runs/{run_name}/overview` - единый обзор состояния запуска:
  ...
  создан или уже удален. Backend может кэшировать собранный overview по
  сигнатуре status artifacts (`size + mtime_ns`); изменение status-файлов
  инвалидирует кэш.
...
- `GET /api/results/runs/{run_name}/file/{file_path}` - безопасное чтение файла
  внутри папки запуска. Symlink, директории, выход за пределы run-директории и
  файлы больше 512 MiB не отдаются как прямой response; для полного набора
  результатов использовать ZIP-архив с отдельными archive-лимитами.
- `GET /api/visualization/{run_name}/html` - HTML-график из
  `plots/profiles_animation.html` с sandbox CSP. Endpoint отдает только обычный
  файл внутри `plots`, не следует по symlink и ограничивает HTML payload
  100 MiB.
- `GET /api/visualization/{run_name}/status` - текстовый статус визуализации из
  `plots/VISUALIZATION_STATUS.txt` с теми же safe-file правилами и inline-лимитом
  2 MiB.
...
`scripts/api_results_performance_smoke.sh` закрепляет limits для results API:
проверяет время ответа и максимальный размер JSON payload у summary/detail,
overview, plots, test-status и test-suite endpoints на наборе временных
run-папок, отдельно проверяет HTML-график и отказ прямого чтения symlink-файла.

Профили `scripts/check_project.sh`:

- `CHECK_PROFILE=fast` - compile, unit, modular smoke, restart, API smoke и UI
  route smoke для быстрых итераций.
- `CHECK_PROFILE=full` - полный текущий проектный gate, используемый по
  умолчанию.
- `CHECK_PROFILE=research` - отдельный verification research gate; по умолчанию
  запускает `docker/run_test.sh all --dry-run`, а при `RESEARCH_DRY_RUN=0`
  выполняет solver-run выбранного `RESEARCH_TEST_NAME`.
...
- Прямые file responses для результатов и визуализации отдают только обычные
  файлы, не следуют по symlink и имеют явные лимиты размера payload.
