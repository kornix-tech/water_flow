...
PFLOTRAN `PERMEABILITY_FUNCTION PCHIP_LIQ`. Табличные данные записываются рядом
с `pflotran.in` во внешний `.dat` файл:

- для влагопроводности: `saturation kr`, где `kr` - относительная
  влагопроводность, безразмерная.

Значения насыщенности должны строго возрастать, а `kr` должен быть монотонным.
Табличное водоудерживание продолжает храниться в SQLite как часть расчета, но
пока не включается в PFLOTRAN deck: smoke-run текущей сборки PFLOTRAN отклонил
`SATURATION_FUNCTION PCHIP` из-за неподдержанной unsaturated extension.
