...
Verification artifacts дополнительно публикуют `failure_stage`:
`generation`, `solver` или `evaluator`, чтобы API/UI и suite CSV различали этап
сбоя без разбора текста лога.
