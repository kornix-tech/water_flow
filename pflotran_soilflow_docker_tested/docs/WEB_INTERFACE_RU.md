...
Web-режим поднимает единый Docker-контейнер с PFLOTRAN/PETSc, Python-скриптами, FastAPI backend, SQLite-хранилищем jobs и собранным React/Vite frontend. Пользователь работает через браузер: загружает XLSX, запускает demo и verification-тесты, смотрит статусы, логи, результаты, графики и скачивает ZIP расчёта.
...
Страница `Исходные данные` редактирует рабочий XLSX `/workspace/input/soilflow_pflotran_demo.xlsx` через API:
...
- кнопка `Сохранить` записывает значения обратно в рабочий XLSX, после чего demo-запуск использует обновлённые параметры;
- кнопка `Сбросить` восстанавливает рабочий XLSX из шаблона, встроенного в образ.
...
В token-режиме frontend хранит токен только в `sessionStorage`, отправляет его в заголовке `Authorization: Bearer ...` и не добавляет токен в URL. Поэтому скачивание XLSX/ZIP/файлов расчёта и открытие HTML-графиков выполняются через authenticated fetch, а не через прямые публичные ссылки.
...
- Редактирование XLSX в браузере пока не реализовано; доступна загрузка и скачивание файла.
